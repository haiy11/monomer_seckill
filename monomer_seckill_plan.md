...
|  **P2**  | 高性能单体秒杀        | 秒杀核心逻辑 + Redis 预扣库存 + Lua 脚本 | Spring Boot、Redis、Lua、JMeter   |
...
| **周二**  | **P2** | 单体秒杀（下） | ① 完善业务逻辑（库存不足、重复抢购、超时回滚）<br>②实现一个简易的后台管理功能<br>③ 用 JMeter 简易压测，确保单体能抗 500+ QPS<br>④排查 AI 辅助解决报错 | 1.5h |
